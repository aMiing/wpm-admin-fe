<template>
  <span>
    <vab-icon title="已选商品" :icon="['fas', 'shopping-cart']" @click="handleOpenThemeBar"></vab-icon>
    ...
    >
      <div class="custom-drawer-content">
        ...
        <div class="custom-count">
          <span>共 <i class="custom-count-num">{{allcartCount}}</i> 件商品， 总价 <i class="custom-count-price">{{allPayPrice}}</i> 元</span>
          <el-tooltip content="清空购物车" placement="top" effect="light">
            <el-button icon="el-icon-delete" circle  @click="clearCart()"></el-button>
          </el-tooltip>
        </div>
    ...
    methods: {
      ...
      addedChange(row) {
        if(Number(row.saled) === 0){
          this.$baseConfirm('你确定要移除当前商品吗', null, () => {
